...
class App {
    ...
    created() {
        ...
        // Reader переопределит после loadSettings; до этого не должно быть обращения без функции.
        this.$root.readerActionByKeyEvent = () => undefined;
    ...
    redirectIfNeeded() {
        ...
        let url = s[1] || '';
        // url может приходить как percent-encoded (и иногда даже дважды encoded через разные редиректы).
        // Liberama reader ожидает обычный http(s) URL.
        for (let i = 0; i < 2; i++) {
            try {
        ...
        if (url) {
            // Keep base path (e.g. /liberama/) so static routing and ws url remain correct.
