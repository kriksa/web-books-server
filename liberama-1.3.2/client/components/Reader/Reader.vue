...
import {
    flushBooksReadingProgressSync,
    parseBooksDownloadId,
    scheduleBooksReadingProgressSync,
    syncGuestReaderProgressCookie,
} from '../../share/booksReadingProgressSync.js';
...
const componentOptions = {
    ...
    watch: {
        ...
        routeParamUrl: function(newValue) {
            ...
            const recentUrl = (recent && recent.url ? recent.url : '');
            if (newValue !== '' && newValue !== 'undefined' && newValue !== recentUrl) {
...
class Reader {
    ...
    created() {
        ...
        this.debouncedSetRecentBook = _.debounce(async(newValue) => {
            ...
            if (recent && (recent.bookPos != newValue || recent.bookPosSeen !== this.bookPosSeen)) {
                ...
                const catalogBookId = parseBooksDownloadId(recent.path || recent.url || '');
                if (catalogBookId) scheduleBooksReadingProgressSync(catalogBookId, newValue);
    ...
    mounted() {
        (async() => {
            ...
            syncGuestReaderProgressCookie();
            this._flushBooksProgress = () => flushBooksReadingProgressSync();
            window.addEventListener('pagehide', this._flushBooksProgress, { capture: true });
    ...
    beforeUnmount() {
        flushBooksReadingProgressSync();
        if (this._flushBooksProgress) {
            window.removeEventListener('pagehide', this._flushBooksProgress, { capture: true });
            this._flushBooksProgress = null;
        }
    }
    ...
    get routeParamUrl() {
        ...
        // Sometimes broken links can yield literal "undefined" — treat as empty.
    ...
    keyHook(event) {
        ...
        // Компонент Reader монтируется только на маршруте чтения; проверка getRootRoute() ломала клавиши
        // при base path /liberama/, редиректах и до готовности vue-router.

        if (!result && this.$refs.dialogs)
        ...
        if (!result && event.type == 'keydown') {
            const map = this.$root.readerActionByKeyEvent;
            const action = typeof map === 'function' ? map(event) : undefined;
