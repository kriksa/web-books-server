...
import {getOrCreateReaderDeviceId} from '../../../share/readerDeviceId';

/** IndexedDB → WebSQL → localStorage (как у эталонного поведения localForage). */
const LF_DRIVERS = [localForage.INDEXEDDB, localForage.WEBSQL, localForage.LOCALSTORAGE];
...
class BookManager {
    async init(settings) {
        ...
        /** Сброс debounce перед закрытием вкладки — иначе последние сотни мс позиции могут не попасть в IndexedDB. */
        if (typeof window !== 'undefined' && !this._flushListenersBound) {
            this._flushListenersBound = true;
            const flush = () => this.flushPendingRecentSave();
            window.addEventListener('pagehide', flush, { capture: true });
            window.addEventListener('beforeunload', flush, { capture: true });
        }
    }

    flushPendingRecentSave() {
        try {
            if (this.saveRecent && typeof this.saveRecent.flush === 'function')
                this.saveRecent.flush();
            if (this.saveRecentItem && typeof this.saveRecentItem.flush === 'function')
                this.saveRecentItem.flush();
        } catch (e) {
            //
        }
    }
    ...
    async setRecentBook(value) {
        ...
        if (!result.readerDeviceId) {
            result.readerDeviceId = getOrCreateReaderDeviceId();
        }
