...
import fb2NormalizeStringFromBytes, { looksLikeFb2Xml } from '../share/fb2NormalizeStringFromBytes';
...
class Reader {
    ...
    async loadCachedBook(url, callback, estSize = -1) {
        ...
        let text;
        if (looksLikeFb2Xml(ab)) {
            text = fb2NormalizeStringFromBytes(ab);
        } else {
            text = new TextDecoder('utf-8', { fatal: false }).decode(new Uint8Array(ab));
        }
        return Object.assign({}, response, { data: text });
    }
