...
function saveReaderLaunchPrefs() {
  try {
    const mode = readerLaunchPrefs.value.mode;
    localStorage.setItem(
      READER_PREFS_KEY,
      JSON.stringify({
        mode,
        embeddedEnabled: mode !== 'external'
      })
    );
  } catch {
    /* ignore */
...
function selectReaderMode(mode, checked) {
  if (!checked) return;
  if (mode !== 'auto' && mode !== 'embedded' && mode !== 'external') return;
  readerLaunchPrefs.value.mode = mode;
  saveReaderLaunchPrefs();
}
...
const formattedLastAdded = computed(() => {
  return formatAnyDate(libraryStats.value?.last_added_at, 'datetime');
});
...
function formatAnyDate(raw, mode = 'datetime') {
  if (raw === null || raw === undefined || raw === '') return mode === 'date' ? 'N/A' : '—';
  const s = String(raw).trim();
  let d;
  // unix timestamp (seconds or milliseconds) sometimes comes as string/number
  if (/^\d+$/.test(s)) {
    const n = Number(s);
  ...
  if (Number.isNaN(d.getTime())) {
    // SQLite DATETIME "YYYY-MM-DD HH:MM:SS" — в Safari часто не парсится как есть
    const isoLike = s.includes('T') ? s : s.replace(' ', 'T');
    d = new Date(isoLike);
  }
  if (Number.isNaN(d.getTime())) return mode === 'date' ? 'N/A' : '—';
      ...
      <form @submit.prevent="saveSettings" class="space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="md:col-span-2 p-4 border border-gray-600 rounded-lg">
            ...
            <div class="mb-4">
              <label class="block text-sm font-medium mb-2 text-gray-300">Выбор читалки (чекбоксы)</label>
              <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <label class="flex items-center gap-2 text-sm font-medium text-gray-300 cursor-pointer select-none">
                  <input
                    :checked="readerLaunchPrefs.mode === 'auto'"
                    @change="selectReaderMode('auto', $event.target.checked)"
                    type="checkbox"
                    class="h-4 w-4 rounded border-gray-600 bg-gray-700 text-indigo-600 focus:ring-indigo-500 focus:ring-offset-gray-800"
                  />
                  Авто
                ...
                <label class="flex items-center gap-2 text-sm font-medium text-gray-300 cursor-pointer select-none">
                  <input
                    :checked="readerLaunchPrefs.mode === 'embedded'"
                    @change="selectReaderMode('embedded', $event.target.checked)"
                    type="checkbox"
                    class="h-4 w-4 rounded border-gray-600 bg-gray-700 text-indigo-600 focus:ring-indigo-500 focus:ring-offset-gray-800"
                  />
                  Внутренняя читалка
                ...
                <label class="flex items-center gap-2 text-sm font-medium text-gray-300 cursor-pointer select-none">
                  <input
                    :checked="readerLaunchPrefs.mode === 'external'"
                    @change="selectReaderMode('external', $event.target.checked)"
                    type="checkbox"
                    class="h-4 w-4 rounded border-gray-600 bg-gray-700 text-indigo-600 focus:ring-indigo-500 focus:ring-offset-gray-800"
                  />
                  Внешняя читалка
              ...
              <p class="text-xs text-gray-400 mt-2">«Авто» и «Внутренняя» — foliate в полноэкранном оверлее (в адресе появляется хэш <code class="bg-gray-900 px-1 rounded">#/read?book=…</code>, как у Liberama с <code class="bg-gray-900 px-1 rounded">#/read?url=…</code>); прямой адрес <code class="bg-gray-900 px-1 rounded">/read?book=…</code> тоже поддерживается. «Внешняя» — открыть URL из поля ниже в новой вкладке.</p>
            </div>
      ...
      <div v-if="libraryStats" class="mt-8 p-4 border border-gray-600 rounded-lg">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 text-sm">
          ...
          <div class="p-3 rounded-lg bg-gray-700/50 col-span-2 sm:col-span-1 break-words">
            ...
            <div class="text-white font-semibold mt-1">{{ formattedLastAdded }}</div>
          </div>
        </div>
      </div>
            ...
            <table class="w-full text-sm">
              ...
              <tbody class="text-white">
                <tr v-for="u in users" :key="u.id" class="border-t border-gray-600 hover:bg-gray-700/50">
                  ...
                  <td class="p-3 text-gray-400">{{ formatAnyDate(u.created_at, 'date') }}</td>
